 ...
 *
 * TO D0:
 *  1. Add functionality for the paths starting on the right side of the martix (n2)
 *  3. Change file and class to something more clever
...
public class Dtw1 {
  ...
  public float[][] acm(float[][] cm) {
    ...
    if (us==1 && ls==1) {
	  ...
	  // slope constraint (P) == 0 (Sakoe, Chiba 1978)
	    acm[i2][i1] = min(acm[i2-1][i1-1],
                              acm[i2-1][i1  ],
                              acm[i2  ][i1-1])
        		    +  cm[i2  ][i1  ];}}
    }
      ...
      for (int i2=2; i2<n2; ++i2) {
        for (int i1=2; i1<n1; ++i1) {
	  // slope constraint (P) == 1 (Sakoe, Chiba 1978)
          acm[i2][i1] = min(acm[i2-1][i1-2]+cm[i2][i1-1]+cm[i2][i1],
                            acm[i2-1][i1-1]+cm[i2][i1],
                            acm[i2-2][i1-1]+cm[i2-1][i1]+cm[i2][i1]);}}
    }
    return acm;
  }
  ...
  public float[][][] paths(float[][] cm) {
    ...
    for (int ip=0; ip<np; ++ip) { 
      int i1 = pi[ip];
      int i2 = n2-1;
      float[][] allp = path(i1,i2,cm,acm,pe[ip],tpi[ip],zpi[ip]); // path data
...
////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  float[] pe, float[] tpi, float[] zpi) 
  {
    ...
    while (i1>1 && i2>1) 
    {
      ...
      float stp2 = acm[i2-2][i1-1]+cm[i2-1][i1  ];
      float stp3 = acm[i2-1][i1-2]+cm[i2  ][i1-1];
