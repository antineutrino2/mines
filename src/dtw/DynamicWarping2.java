...
public class DynamicWarping2 {
   ...
   */
  public DynamicWarping2(int shiftMin, int shiftMax) {
   ...
   */
   ...
   public float[][] computeErrorsOld(float[] f, float[] g) {
    int n1 = f.length;
    int n2 = g.length;
    int nl = _nl;
    int n1m = n1-1;
    ...
    float[][] e = new float[n1][nl];
    // n2 > n1
    // nl = 1+lmax-lmin = 1+n2-n1+(ff-ff+1-1) 
    // 0 <= il < nl, where il is index for lag
    // 0 <= i1 < n1, where i1 is index for sequence f
    // 0 <= i2 < n1, where i2 index for sequence g
    // i2 = i1+il+lmin, where il+lmin = lag
    // 0 <= i1+il+lmin < n1, so that i2 is in bounds
    // max(0,-lmin-i1) <= il < min(nl,n1-lmin-i1)
    // max(0,-lmin-il) <= i1 < min(n1,n1-lmin-il)
    // i2 = 0    => i1 = -lmin-il
    // i2 = n2-1 => i1 = n1-1-lmin-il
    for (int i1=0; i1<n1; ++i1) {
      int illo = max(0,-_lmin-i1); // see notes
      int ilhi = min(nl,n1-_lmin-i1); // above
      // lower triangle
      for (int il=0,i2=i1+il+_lmin; il<illo; ++il,++i2) 
        e[i1][il] = (i2>=0) ? error(f[i1],g[i2]) : error(f[0],g[-_lmin-il]); 
      // indicies in bounds for both f and g
      for (int il=illo,i2=i1+il+_lmin; il<ilhi; ++il,++i2)
        e[i1][il] = error(f[i1],g[i2]);
      // upper triangle
      for (int il=ilhi,i2=i1+il+_lmin; il<nl; ++il,++i2)
        e[i1][il] = (i2<n2) ? error(f[i1],g[i2]) : error(f[n1m],g[n2m-_lmin-il]);
    }
    return e;
  }

  public float[][] computeErrors2(float[] f, float[] g) {
    int n1 = f.length;
    int n2 = g.length;
    int nl = _nl;
    int n1m = n1-1;
    int n2m = n2-1;
    float[][] e = new float[n2][nl];
    // Notes for indexing:
    // 0 <= il < nl, where il is index for lag
    // 0 <= i1 < n1, where i1 is index for sequence f
    // 0 <= i2 < n2, where i2 is index for sequence g
    // i1 = i2+il+lmin, where il+lmin = lag
    // 0 <= i2+il+lmin < n2, so that i2 is in bounds
    // max(0,-lmin-i1) <= il < min(nl,n2-lmin-i2)
    // max(0,-lmin-il) <= i2 < min(n2,n2-lmin-il)
    // i1 = 0    => i2 =     -lmin-il
    // i1 = n1-1 => i2 = n2-1-lmin-il
    for (int i2=0; i2<n2; ++i2) {
      int illo = min(nl-1,max(0,-_lmin-i2)); // see notes
      int ilhi = max(0,min(nl,n2-_lmin-i2)); // above
      for (int il=0,i1=i2+il+_lmin; il<illo; ++il,++i1)
        e[i2][il] = (i1>=0) ?
          error(f[i1],g[i2]) : 
          error(f[0],g[-_lmin-il]);
      for (int il=illo,i1=i2+il+_lmin; il<ilhi; ++il,++i1)
        e[i2][il] = error(f[i1],g[i2]);
      for (int il=ilhi,i1=i2+il+_lmin; il<nl; ++il,++i1)
        e[i2][il] = (i1<n1) ?
          error(f[i1],g[i2]) : 
          error(f[n1m],g[n2m-_lmin-il]);
    }
    return e;
  }
...
/*  private void computeErrorsFrac(float[] f, float[] g, float[][] e) { 
    ...
    si.interpolate(n1,1.0f,0.0f,g,nx,_shifts.getDelta(),0.0,gi);
