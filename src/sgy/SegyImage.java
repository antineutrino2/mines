 ...
 * to the crossline direction. For 3D images the 3rd dimension corresponds 
 ...
 * crossline, and inline dimensions, respectively. The minimum crossline 
 * and inline indices i2min and i3min in SEG-Y trace headers are 
 * typically non-zero. 
 * <p>
...
public class SegyImage {
  ...
  public void printAllInfo() {
    ...
    System.out.printf(
      "  i2min = %5d, i2max = %5d (crossline indices)%n",
      _i2min,_i2max);
    System.out.printf(
      "  i3min = %5d, i3max = %5d (inline indices)%n",
      _i3min,_i3max);
    System.out.printf(
      "  xmin = %11.6f, xmax = %11.6f (x coordinates)%n",
      _xmin,_xmax);
    System.out.printf(
      "  ymin = %11.6f, ymax = %11.6f (y coordinates)%n",
      _ymin,_ymax);
    System.out.println("grid sampling:");
    System.out.printf(
      "  n1 = %5d (number of samples per trace)%n",_n1);
    System.out.printf(
      "  n2 = %5d (number of traces in crossline direction)%n",_n2);
    System.out.printf(
      "  n3 = %5d (number of traces in inline direction)%n",
      _n3);
    System.out.printf(
      "  d1 = %8.6f (time sampling interval, in s)%n",_d1);
    System.out.printf(
      "  d2 = %8.6f (crossline sampling interval)%n",_d2);
    System.out.printf(
      "  d3 = %8.6f (inline sampling interval)%n",_d3);
  ...
  /**
   * Sets bytes used to find crossline and xline numbers in trace headers.
   * Both crossline and xline numbers are represented by four-byte integers, 
   * but these integers are not always placed in the standard bytes
   * 189-192 (for crossline) and 193-196 (for xline) of trace headers. 
   ...
   * for SEG-Y format.
   * @param crosslineByte first byte of crossline number.
   * @param xlineByte first byte of xline number.
   */
  public void setInlineXlineBytes(int crosslineByte, int xlineByte) {
    _i2hi = (xlineByte-1)/4;
    _i3hi = (crosslineByte-1)/4;
  }
  /**
   * Sets bytes used to find X and Y numbers in trace headers.
   * Both X and Y numbers are represented by four-byte integers, 
   * but these integers are not always placed in the standard bytes
   * 73-76 (for X) and 77-80 (for Y) of trace headers. 
   * This method allows these standard byte locations to be overridden.
   * <p>
   * Note that the first byte is 1, not 0, as in the documentation 
   * for SEG-Y format.
   * @param xByte first byte of X coordinate.
   * @param yByte first byte of Y coordinate.
   */
  public void setXYBytes(int xByte, int yByte) {
    _xhi = (xByte-1)/4;
    _yhi = (yByte-1)/4;
  }
  /**
	 ...
	 float[][][] f = new float[_n3][_n2][_n1];
	 	for (int i3=0; i3<_n3; ++i3) {
	 		for (int i2=0; i2<_n2; ++i2) {
	 			f[i3][i2] = getTrace(i2+_i2min,i3+_i3min);
	 		}
	 	}
	 return f;
  }
  ...
  /**
   * Gets the x and y coordinates for all crossline/inlines
   * @return the coordinate mapping
   */
  public double[][][] getCoords() {
	  int n3 = _n3;
	  int n2 = _n2;
	  double[][] x = new double[n3][n2];
	  double[][] y = new double[n3][n2];
	  for (int i3=0; i3<n3; ++i3) {
		  for (int i2=0; i2<n2; ++i2) {
			 x[i3][i2] = getX(i2+_i2min,i3+_i3min); 
			 y[i3][i2] = getY(i2+_i2min,i3+_i3min);
		  }
	  }
  ...
  public double[] getCorners() {
	  double cx1 = getX(_i2min,_i3min);
	  double cy1 = getY(_i2min,_i3min);
	  double cx2 = getX(_i2min,_i3max);
	  double cy2 = getY(_i2min,_i3max);
	  double cx3 = getX(_i2max,_i3max);
	  double cy3 = getY(_i2max,_i3max);
	  double cx4 = getX(_i2max,_i3min);
	  double cy4 = getY(_i2max,_i3min);
	  return new double[]{cx1,cy1,cx2,cy2,cx3,cy3,cx4,cy4};
  }
  
  public double[] getCoordInfo() {
      loadTraceHeaderInfo();
	  return new double[]{_xref,_yref,
			  		  	_i2ref,_i3ref,
			  		  	_d2,_d3,
			  		  	_sin2,_sin3,
			  		  	_cos2,_cos3};
  ...
  public void writeFloats(
    ...
    int i1min, int i1max, int i2min, int i2max, int i3min, int i3max)
  {
    writeFloats(fileName,scaleFactor,i1min,i1max,i2min,i2max,i3min,i3max,false);
  }
   ...
   * @param gathers if gathers, then ignore grid checks.
   */
  public void writeFloats(
    String fileName,
    double scaleFactor,
    int i1min, int i1max,
    int i2min, int i2max,
    int i3min, int i3max, boolean gathers)
  {
    loadTraceHeaderInfo();
    ...
    try {
      ...
      for (int i3=i3min; i3<=i3max; ++i3) {
        ...
        if (!gathers) {
          for (int i2=i3+i2min; i2<=i3+i2max; ++i2) {
            getTrace(i2,i3,f);
            copy(m1,i1min,f,0,g);
            if (s!=1.0f) {
  ...
  private int _xhi = 45; // index in trace header of integer xline number
  private int _yhi = 46; // index in trace header of integer iline number
  ...
  private static String[] _traceHeaderFields = {
    ...
    "189 - 192: 3D crossline number, the line number",
    "193 - 196: 3D inline number, the trace within line",
  ...
  private void loadBinaryHeaderInfo() {
    ...
    System.out.println(_nbyte-3600L);
    System.out.println(_ntrace*(240L+_bytesPerSample*n1));
    ...
    try {
      ...
      for (int itrace=0; itrace<_ntrace; ++itrace) {
        ...
        if (pxy>0) // if positive, multiply
          ...
          sxy /= -pxy;
        double x = hi[_xhi]*sxy; // x coordinate 
        double y = hi[_yhi]*sxy; // y coordinate 
  ...
  public static float[][][] cmpSort(
    String fileName,
    Sampling ss, Sampling sr,
    float[][][] x1, float[][][] x2,
    float[][][] x3, float[][][] x4) 
  {
    // combine shots
    float[][][] x = new float[ss.getCount()][sr.getCount()][x1[0][0].length];
    ...
    for (int i=0; i<x4.length; ++i)
      x[i] = x4[i];
    return cmpSort(fileName,ss,sr,x);
  }
    public static float[][][] cmpSort(
    String fileName,
    Sampling ss, Sampling sr,
  ...
  public static float[][][] cmpSort(
    String fileName, 
    Sampling ss, Sampling sr, float[][][] x) 
  {
    int ns = ss.getCount();
    int nr = sr.getCount();
    int nt = x[0][0].length;
    ...
    double dr = sr.getDelta();
    double dx = ds*2.0; 
    double fx = ss.getFirst();
    double fr = sr.getFirst();
    int nx = (int)(nr*dr/dx);
    Sampling sx = new Sampling(nx,dx,fr);
    double dm = dr*0.5;
    int nm = (int)(ns*(dx/dr)+2*nr-1);
    Sampling sm = new Sampling(nm,dm,0);//sr.getFirst());
    double vml = sm.getLast();
    // sort shot traces to cmp
    float[][][] y = new float[nm][nx][nt];
    //float[] f = new float[nt];
    try {
      ArrayOutputStream aos = new ArrayOutputStream(fileName);
      int mb = (int)(0.5+4.0*nm*nx*nt/1.0e6);
      System.out.print(
        "writing "+nm+"*"+nx+"*"+nt+" floats ("+mb+" MB) ");
      for (int im=0; im<nm; ++im) {
        double vm = sm.getValue(im);
        double fs = ss.getValueExtended((int)ceil(vm/ds));
        if (fs>ss.getLast()) continue;
        for (int ix=0; ix<nx; ++ix) {
          double vx = sx.getValue(ix);
          //if (vm-vx<0.0) continue;
          if (vm+vx>vml) continue;
          //if (fs-vm-vx>0.0) continue;
          int is = ss.indexOfNearest(vm+vx);
          int ir = sr.indexOfNearest(fs-vm-vx);
          y[im][ix] = x[is][ir];
          //f = x[is][ir];
          //aos.writeFloats(f);
        }
  ...
  public static void trace(double x) {
    System.out.println(x);
  }
}
