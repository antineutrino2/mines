...
def getGathers():
  ...
  """
  global s1,so,s2
  n1,no,n2 = 1151,100,4799#2901#cmps2#3599-cmps#12901
  ...
  g = readImage(image,anidir+"cmps3")
  return g
...
def fixOffset(so,gat,xmax):
  ...
  if xmax: 
      ...
      print 'new xmax is',lx 
  so = Sampling(so.count-int(round((lx-so.first)/so.delta)),so.delta,lx)
