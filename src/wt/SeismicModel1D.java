...
public class SeismicModel1D {
  ...
  public void addSensor(double z) {
	...
	/**
	 * Imports arrays of velocity, density and q to layers.
	 * @author Andrew Munoz, CSM
	 */
	public void setLayers(Sampling sz, float[] v, float[] d, float[] q) { 
		int nz = sz.getCount();
		double dz = sz.getDelta();
		double fz = sz.getFirst();
  	//checklengths(nz,v,d,q); 
		addLayer(0.0,(double)v[0],(double)d[0],(double)q[0]);
		for (int iz=0; iz<nz; ++iz) 
			addLayer(iz*dz+fz,(double)v[iz],(double)d[iz],(double)q[iz]);
	}
	public void setLayers(Sampling sz, float[] v, float[] d, float q) { 
	  setLayers(sz,v,d,fillfloat(q,v.length));
	}

	/**
	 * Removes direct arrival at time zero
	 */
	 public void removeSourceSignature(boolean waveletRemoval) {
	 	 _waveletRemoval = waveletRemoval;
	 }
  ...
  public float[][] makeSeismograms(int nt, double dt, double fref) {
    ...
    float[] scal = new float[nt];
		float oscl = (float)(1.0f/(_mult*ntfft));
    for (int it=0; it<nt; ++it)
      scal[it] = (float)(exp(eps*it*dt)*oscl);
    for (int ir=0; ir<nr; ++ir) {
      ...
      for (int it=0; it<nt; ++it)
		...
		// Removes the direct arrival at t=0 for ricker wavelet
		if (_waveletRemoval) {
			if (_fpeak>0.0) {
				float[] w = getRickerWavelet(_fpeak,dt);
				int hw = (w.length-1)/2;
				int ne = min(hw,nt);
				float omult = (float)(1.0/_mult);
    		for (int ir=0; ir<nr; ++ir) {
					for (int it=0; it<ne; ++it) 
    	    	s[ir][it] -= w[hw+it]*omult;
				}	
  		}
		}
		return s;
  ...
  private boolean _waveletRemoval; // true if direct arrival at t=0 is removed
  ...
  private void makeWaves(Cdouble cw, double wref) {
    ...
    double rs = abs(_surfaceRC)*_mult;
    if (_sensorType==SensorType.PRESSURE)
      rs = -rs;
	...
	private float[] getRickerWavelet(double fp, double dt) {
  	double scale = PI*fp*dt;
  	int i0 = (int)(10.0/scale);
  	int nt = 1+2*i0;
  	float[] w = new float[nt];
		for (int it=0; it<nt; ++it) {
    	double x = scale*(it-i0);
    	w[it] = (float)((1.0-2.0*x*x)*exp(-x*x));
		}
  	return w;
	}
  ...
  private static void goTest0() {
    ...
    sm.addLayer(0.0,1.0,1.0,1.0e6);
    sm.addLayer(20.0,2.0,2.0,1.0e6);
    sm.addLayer(40.0,3.0,3.0,1.0e6);
    sm.addSource(0.00,1.0);
    sm.addSensor(0.00);
    sm.dumpLayers();
    int nt = 101;
    double dt = 1.0;
  ...
  private static void goTest1() {
    ...
    sm.setSensorType(SeismicModel1D.SensorType.HYDROPHONE);
    sm.setSurfaceReflectionCoefficient(1.0);
    sm.addLayer(0.0,1.0,1.0,50.0);
    sm.addLayer(20.0,2.0,2.0,50.0);
    sm.addLayer(40.0,3.0,3.0,50.0);
    sm.addSource(0.00,1.0);
    sm.addSensor(0.00);
    sm.dumpLayers();
    int nt = 101;
