...
def goGathers():
  ...
  mfname = basedir+"dat/ani/cmps2.dat"
  makeCmps(fname,x1,mfname=mfname)
...
def goGats(which,makeCmp=False):
  ...
  if firstLook:
    ...
    si.printTraceHeader(0)
    si.printTraceHeader(1)
    si.printTraceHeader(799)
    si.printTraceHeader(800)
    si.printTraceHeader(801)
    si.printTraceHeader(802)
    si.printTraceHeader(803)
    #plotIbmIeeeFloats(si)
  if secondLook:
    si.printAllInfo()
    #plot23(si)
...
def makeCmps(fname,x1,x2=None,x3=None,x4=None,mfname=None):
  ds = 50
  ns = 1641
  fr = -10037.5
  lr = -37.5
  ...
  ss = Sampling(ns,ds,0)
  ns1 = len(x1)
  ss1 = Sampling(ns1,ds,0)
  #SegyImage.cmpSort(fname,ss1,sr,x1);
  nx = int((nr*dr)/(ds*2.0))
  nm = int(ns1*((ds*2)/dr)+2*nr-1)#-(fs-ds*2)/(dr*0.5))
  nt = 1151
  x = readImage(fname,nt,nx,nm)
  mnt = 1151; mnm = 2901; mnx = 100;
  s1m = Sampling(mnt,0.008,0)
  s2m = Sampling(mnx,ds*2,fr)
  s3m = Sampling(mnm,dr*0.5,0)
  mgx = readImage(mfname,mnt,mnx,mnm)
  s1 = Sampling(nt,0.008,0)
  s2 = Sampling(nx,ds*2,fr)
  s3 = Sampling(nm,dr*0.5,0)
  #show3d(x,1.0,s1,s2,s3)
  show3d(mgx,1.0,s1m,s2m,s3m)
  show3d(x1,1.0,s1,sr,ss1)
  return
  slices = [200,300,400,550,1550,2550]
  for si in slices:
    ssi = str(si)
    show2d(x[si],clip=1.0,title=ssi)
    show2d(mgx[si],clip=1.0,title=ssi)
...
def show3d(f,clip=None,s1=None,s2=None,s3=None):
