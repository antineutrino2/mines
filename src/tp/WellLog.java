...
public class WellLog {
  ...
  public static class Data {
    ...
    public static Data readBinary(String fileName) {
      ...
      try {
        ...
        for (int ilog=0; ilog<nlog; ++ilog) {
          ...
          log.x2 = new float[n];
          log.x3 = new float[n];
          ais.readFloats(log.x1); // depth
          ais.readFloats(log.x2); // inline
          ais.readFloats(log.x3); // crossline
          data.add(log);
        }
        ais.close();
        return data;
    ...
    /**
     * Gets well log IDs with the specified curve combination.
     * @param curve1 the well log curve1.
     * @param curve2 the well log curve2 (optional).
     * @param curve3 the well log curve3 (optional).
     * @param curve4 the well log curve4 (optional).
     * @return list of log IDs.
     */
    public List<Long> getLogsWithCombo(
      String curve1, String curve2, String curve3, String curve4) {
      List<Long> list = new ArrayList<Long>();
      for (WellLog log:_data.values()) {
        if (log.getCurve(curve1)!=null) {
	  if (curve2==null && curve3==null && curve4==null) list.add(log.id);
          if (curve2!=null && log.getCurve(curve2)!=null) {
	    if (curve3==null && curve4==null) list.add(log.id);
            if (curve3!=null && log.getCurve(curve3)!=null) {
	      if (curve4==null) list.add(log.id);
              if (curve4!=null && log.getCurve(curve4)!=null) {
                list.add(log.id);
	      }
	    }
	  }
     ...
     * null value in the raster image.
  ...
  public void setCoordinates(long id, String dsFileName) {
    DirectionalSurvey.Data dsdata = new DirectionalSurvey.Data(dsFileName);
    DirectionalSurvey ds = (dsdata!=null)?dsdata.get(id):null;
    if (ds!=null) {
      ds.setCoordinates(this);
    } else {
