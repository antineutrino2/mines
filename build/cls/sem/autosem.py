...
def hypPlots(pdata):
  s1,so,g,vnmo,eta,wb,pduv,uv,up,vnmop,sv,q,ps = pdata
  print 'plotting'
  cmplims=[-6.5,s1.first,so.last,s1.last]
  plotModelsCmp(g,s1,so,vnmo,eta,'modelscmp',clims=[cmplims[0],cmplims[2]])
  plot1(pduv,s1,name='vnmo % difference dp')
  plot1(vnmo,s1,uv,name="vnmo and vnmo estimated")
  plot2(ps,s1,sv,u=vnmop,u2=up,name='semblance comp',sem=True,size=[728,753],colm=3)
  plot2(ps,s1,sv,u=up,name='semblance est',sem=True,size=[728,753])
  plot2(ps,s1,sv,name='semblance',sem=True,size=[728,753],colm=3)
  plot2(q,s1,so,name='flat cmp',cmp=True,perc=99)
  plot2(g,s1,so,name='cmp',cmp=True,perc=99,lims=cmplims)
...
def nonHypV(data):
  ...
  p1min,p1max = mul(p1min,DvDt),mul(p1max,DvDt)
  p2min,p2max = mul(p2min,DeDt),mul(p2max,DeDt)
  plot1(vl1,s1,vl2,name="vnmo interval bounds")
  plot1(el1,s1,el2,name="eta interval bounds")
  ...
  plot1(floats(p2min),s1,floats(p2max),dedt,name="eeta derivative bounds")
  # Test Data
  #gt = Gather(s1,so,sv,se)
  ...
  vnmop = vnmo
  etap = eta
  pdata = s1,so,g,vnmo,eta,wb,pduv,uv,ue,up1,up2,vnmop,etap,sv,se,q,qe,ps,cgs,vel,veu,eel,eeu
...
def nonHypPlots(pdata):
  s1,so,g,vnmo,eta,wb,pduv,uv,ue,up1,up2,vnmop,etap,\
    sv,se,q,qe,ps,cgs,vel,veu,eel,eeu = pdata
  vve=True
  print 'plotting'
  ...
  plot2(ps[100],s1=sv,s2=se,name='vnmo vs eta t=100',vve=vve,mx=[up1[100],up2[100],vnmop[100],etap[100]])
  plot2(ps[400],s1=sv,s2=se,name='vnmo vs eta t=400',vve=vve,mx=[up1[400],up2[400],vnmop[400],etap[400]])
  plot2(ps[500],s1=sv,s2=se,name='vnmo vs eta t=500',vve=vve,mx=[up1[500],up2[500],vnmop[500],etap[500]])
  plot2(ps[750],s1=sv,s2=se,name='vnmo vs eta t=750',vve=vve,mx=[up1[750],up2[750],vnmop[750],etap[750]])
  plot2(ps[900],s1=sv,s2=se,name='vnmo vs eta t=900',vve=vve,mx=[up1[900],up2[900],vnmop[900],etap[900]])
  plot2(ps[1100],s1=sv,s2=se,name='vnmo vs eta t=1100',vve=vve,mx=[up1[1100],up2[1100],vnmop[1100],etap[1100]])
  plot2(q,s1,so,name='flat cmp',cmp=True,perc=98)
  plot2(qe,s1,so,name='flat real cmp',cmp=True,perc=98)
  plot2(g,s1,so,name='cmp',cmp=True,perc=99,lims=cmplims)
  plot3(ps,cmap=jet,s1=sv,s2=se,s3=s1,u=[up1,up2,ut],perc=99,\
                                      name='eta semblance',paper='bpnhsu')
  plot3(ps,cmap=jet,s1=sv,s2=se,s3=s1,name='eta semblance')
  plot3(ps,cmap=jet,s1=sv,s2=se,s3=s1,u=[vnmop,etap,ut],name='eta semblance exact')
...
def getData(ix2,stack=None,xmax=None):
  ...
  plot2(mul(vpz,0.001),s1z,s2,name='vpz',vpz=True,paper="vpz",lims=lims,cbar="velocity km/s",cmppts=[ix2,so])
  plot2(epz,s1z,s2,name='epz',vpz=True,paper="epz",lims=lims,cbar="epsilon")
  plot2(dez,s1z,s2,name='dez',vpz=True,paper="dez",lims=lims,cbar="delta")
  return s1,so,g,vrms,eeta,wb,vp
...
def plot1(x,s1=None,u=None,v=None,name=None,hlabel=None,cgs=None,paper=None,colm=1):
...
def plot2(x,s1=None,s2=None,u=None,u2=None,cmap=gray,\
  ...
  if cmppts:
    ...
    no = so.count
    cmpl = zerofloat(no)
    x1 = fillfloat(s1.delta*3,no)
    for i in range(1):
      cmpl[i] = x2#+so.getValue(i)
...
def plot3(x,cmap=jet,s1=None,s2=None,s3=None,u=None,\
  ...
  if u:
    ...
    if s3:
      ...
      us = zerofloat(nt)
      for i in range(nt):
        us[i] = x[i][s2.indexOfNearest(u[1][i])][s1.indexOfNearest(u[0][i])]
      plot1(us,s3,name="semblance along path "+name)
  sf = SimpleFrame(world)
  ov = sf.getOrbitView()	
  ov.setAxesScale(0.25,2.0,0.50)
  ov.setScale(2.0)
  ipg.setSlices(50,20,400)
  if name:
    sf.setTitle(name)
  sf.setVisible(True)
  if paper:
...
def frame(panel,name,size=[800,1000],paper=None,colm=None):
